...
newroot = ET.Element("xml")

for child in root.iter('name'):
    print child.text
    doc = minidom.parse('/Volumes/C/Users/gordonl/Documents/public_html/runs/' + urllib2.unquote(child.text))
    ...
    run = ET.SubElement(newroot, "file")
    ET.SubElement(run, "name").text = child.text
